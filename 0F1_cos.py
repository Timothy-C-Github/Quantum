# -*- coding: utf-8 -*-
"""
Created on Wed Jan 15 21:16:27 2025
...
# Generate training data: x values starting at -1 and ends at 1; and corresponding cosine values
x_values = np.linspace(-1, 1, 101)  # Range for x starting at -1
y_true = np.cos(x_values)  # Target: cosine of x
...
def optimize_z_for_b(b, x_values, y_true):
    ...
    initial_z = np.linspace(-100.0, 100.0, len(x_values))  # Linear initial guesses for z
...
b_values = np.linspace(-100.0, 100.0, 1001)  # Test different b values from 0.1 to 2.0
...
# Plot the results
plt.plot(x_values, y_true, label='cos(x)', color='blue', linestyle='-', linewidth=2)
plt.plot(x_values, y_pred, label='Optimized 0F1 Fit', color='red', linestyle='--', linewidth=2)
plt.legend()
plt.xlabel('x')
plt.ylabel('Value')
plt.title('Fitting 0F1 to cos(x)')
plt.show()
